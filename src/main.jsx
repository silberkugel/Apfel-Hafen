...
const translations = {
  de: {
    ...
    noPorts: "Keine Ports veröffentlicht.", cpu: "CPU-Kerne", memory: "Arbeitsspeicher", containerInfo: "Container-Informationen", tools: "Werkzeuge", darkMode: "Darstellung wechseln", inspectJson: "JSON untersuchen",
  },
  en: {
    ...
    noPorts: "No ports published.", cpu: "CPU cores", memory: "Memory", containerInfo: "Container information", tools: "Tools", darkMode: "Toggle appearance", inspectJson: "Inspect JSON",
  },
};

const emptyDraft = () => ({ name: "", image: "", cpus: "2", memoryMb: "1024", architecture: "auto", rosetta: false, dnsDomain: "", network: "", interactive: false, tty: false, workdir: "", entrypoint: "", readOnlyRoot: false, useInit: true, ssh: false, start: true, arguments: [], ports: [], volumes: [], variables: [] });
...
function App() {
  ...
  const [theme, setTheme] = useState(() => localStorage.getItem("apfel-hafen.theme") || (window.matchMedia?.("(prefers-color-scheme: dark)").matches ? "dark" : "light"));
  ...
  const [containerInspection, setContainerInspection] = useState(null);
  ...
  async function inspectContainer(container) {
    if (!auth.authenticated) return setLoginOpen(true);
    setContainerMenuOpen(null); setBusy(`${container.name}:inspect`);
    try {
      const data = await request(`/api/containers/${encodeURIComponent(container.name)}/inspect`);
      setContainerInspection({ name: container.name, value: data.inspection });
    } catch (err) { setError(err.message); } finally { setBusy(""); }
  }
  ...
  return <main>
    ...
    <nav className="service-tabs" aria-label={t("serviceCenter")}><button className={section === "containers" ? "active" : ""} onClick={() => { setSection("containers"); setQuery(""); }}>◇ {t("containers")} <span>{containers.length}</span></button><button className={section === "tools" ? "active" : ""} onClick={() => { setSection("tools"); setQuery(""); }}>⌘ {t("tools")}</button><button className={section === "launchd" ? "active" : ""} onClick={() => { setSection("launchd"); setQuery(""); }}>⚙ {t("launchd")} <span>{services.length}</span></button></nav>
...
<div hidden={section !== "tools"}>{section === "tools" && <div className="panel">{error && <div className="message error" role="alert">{error}</div>}{notice && <div className="message success" role="status">{notice}</div>}</div>}<OperationsConsole active={section === "tools"} request={request} language={language} authenticated={auth.authenticated} onNotice={setNotice} onError={setError} /></div>
    ...
    {createOpen && <Modal extraWide onClose={() => !createBusy && setCreateOpen(false)}><p className="eyebrow">{t("create")} · {createStep}/3</p><h2>{t("createTitle")}</h2><div className="step-indicator"><span className={createStep >= 1 ? "active" : ""}>{t("general")}</span><span className={createStep >= 2 ? "active" : ""}>{t("networkStorage")}</span><span className={createStep >= 3 ? "active" : ""}>{t("variablesReview")}</span></div>{createError && <div className="login-error">{createError}</div>}
      ...
      {createStep === 1 && <AdvancedContainerOptions draft={draft} setDraft={setDraft} language={language} />}
    ...
    {containerInspection && <Modal extraWide onClose={() => setContainerInspection(null)}><h2>{containerInspection.name} · Inspect</h2><button onClick={() => navigator.clipboard.writeText(JSON.stringify(containerInspection.value, null, 2)).catch((err) => setError(err.message))}>{language === "de" ? "JSON kopieren" : "Copy JSON"}</button><pre className="diagnostic-output">{JSON.stringify(containerInspection.value, null, 2)}</pre></Modal>}
    {containerLogs && <Modal extraWide onClose={() => setContainerLogs(null)}><p className="eyebrow">{t("containerLogsLabel")}</p><h2>{containerLogs.name}</h2><LiveLogViewer enabled={auth.authenticated} url={`/api/containers/${encodeURIComponent(containerLogs.name)}/logs/stream`} language={language} title={containerLogs.name} /><div className="log-sections"><section><h3>{t("bootLog")}</h3><pre>{containerLogs.bootLog || t("noLogOutput")}</pre></section><section><h3>{t("containerOutput")}</h3><pre>{containerLogs.outputLog || t("noLogOutput")}</pre></section></div><div className="log-actions"><button className="cancel" onClick={() => setContainerLogs(null)}>{t("close")}</button><button className="primary" disabled={busy === `${containerLogs.name}:logs`} onClick={() => openContainerLogs({ name: containerLogs.name })}>↻ {t("reload")}</button></div></Modal>}
