...
function App() {
  ...
  const [administration, setAdministration] = useState({ enabled: false, volumeBasePath: "", listenHost: "127.0.0.1", loading: false, saving: false, error: "", success: "" });
  ...
  return <main>
    ...
    {administrationOpen && <Modal wide onClose={() => setAdministrationOpen(false)}>
      ...
      <section className="setting-row"><div><strong>{t("autostart")}</strong><span>{t("autostartHelp")}</span></div><button className={`switch ${administration.enabled ? "on" : ""}`} onClick={() => setAdministration((v) => ({ ...v, enabled: !v.enabled, success: "" }))} aria-pressed={administration.enabled}><i /></button></section>
