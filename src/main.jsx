...
const emptyLaunchdDraft = () => ({ label: "", program: "", argumentsText: "", runAtLoad: true, keepAlive: false, startInterval: "", calendarIntervals: [], loadNow: true, standardOutPath: "", standardErrorPath: "" });
...
function App() {
  ...
  async function createService(event) {
    ...
    try {
      const payload = {
        ...serviceDraft,
        arguments: serviceDraft.argumentsText.split("\n").map((value) => value.trim()).filter(Boolean),
        startInterval: serviceDraft.startInterval === "" ? null : Number(serviceDraft.startInterval),
        calendarIntervals: serviceDraft.calendarIntervals.map((entry) => {
          const [hour, minute] = entry.time.split(":").map(Number);
          return { weekday: entry.weekday === "" ? null : Number(entry.weekday), hour, minute };
        }),
      };
      delete payload.argumentsText;
      const data = await request("/api/services/launchd", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      setServiceCreateOpen(false); setServiceDraft(emptyLaunchdDraft()); setNotice(data.message); await loadServices();
  ...
  return <main>
    ...
    {serviceInfo && <Modal wide onClose={() => setServiceInfo(null)}><p className="eyebrow">{t("launchdInfo")}</p><h2>{serviceInfo.label}</h2><dl className="info-grid"><div><dt>{t("configuration")}</dt><dd><code>{serviceInfo.path}</code></dd></div><div><dt>{t("scope")}</dt><dd>{t(serviceInfo.scope === "user" ? "userScope" : "systemScope")} · {serviceInfo.kind}</dd></div><div><dt>Status</dt><dd>{serviceInfo.running ? t("running") : serviceInfo.loaded ? t("loaded") : t("unloaded")}{serviceInfo.pid ? ` · PID ${serviceInfo.pid}` : ""}</dd></div><div><dt>{t("schedule")}</dt><dd>{launchdScheduleLabels(serviceInfo, t).join(" · ") || t("manualStart")}</dd></div></dl><section className="info-section"><h3>Programm</h3><code>{serviceInfo.program || "–"}</code>{serviceInfo.programArguments.length > 1 && <p>{serviceInfo.programArguments.slice(1).join(" ")}</p>}</section><section className="info-section"><h3>{t("logs")}</h3><p>{serviceInfo.standardOutPath || "stdout: –"}<br />{serviceInfo.standardErrorPath || "stderr: –"}</p></section><div className="info-close"><button className="primary" onClick={() => setServiceInfo(null)}>{t("close")}</button></div></Modal>}
    {serviceCreateOpen && <Modal wide onClose={() => !serviceCreateBusy && setServiceCreateOpen(false)}><form className="service-create-form" onSubmit={createService}><p className="eyebrow">{t("serviceCenter")}</p><h2>{t("createService")}</h2><p>{t("createServiceIntro")}</p>{serviceCreateError && <div className="login-error">{serviceCreateError}</div>}<label className="form-field"><span>{t("label")}</span><input autoFocus required value={serviceDraft.label} onChange={(e) => setServiceDraft((value) => ({ ...value, label: e.target.value }))} placeholder="de.meinname.mein-dienst" /></label><label className="form-field"><span>{t("programPath")}</span><input required value={serviceDraft.program} onChange={(e) => setServiceDraft((value) => ({ ...value, program: e.target.value }))} placeholder="/usr/local/bin/mein-programm" /></label><label className="form-field"><span>{t("arguments")}</span><textarea value={serviceDraft.argumentsText} onChange={(e) => setServiceDraft((value) => ({ ...value, argumentsText: e.target.value }))} placeholder={`--port\n8080`} /><small>{t("onePerLine")}</small></label><div className="service-log-fields"><label className="form-field"><span>{t("stdoutPath")}</span><input value={serviceDraft.standardOutPath} onChange={(e) => setServiceDraft((value) => ({ ...value, standardOutPath: e.target.value }))} placeholder="/Users/…/Library/Logs/dienst.log" /></label><label className="form-field"><span>{t("stderrPath")}</span><input value={serviceDraft.standardErrorPath} onChange={(e) => setServiceDraft((value) => ({ ...value, standardErrorPath: e.target.value }))} placeholder="/Users/…/Library/Logs/dienst-error.log" /></label></div><LaunchdScheduleOptions draft={serviceDraft} setDraft={setServiceDraft} t={t} /><label className="load-now-option"><input type="checkbox" checked={serviceDraft.loadNow} onChange={(e) => setServiceDraft((value) => ({ ...value, loadNow: e.target.checked }))} /> {t("loadNow")}</label><div className="wizard-actions"><button type="button" className="cancel" disabled={serviceCreateBusy} onClick={() => setServiceCreateOpen(false)}>{t("cancel")}</button><button type="submit" className="primary" disabled={serviceCreateBusy || !serviceDraft.label.trim() || !serviceDraft.program.trim() || (serviceDraft.startInterval !== "" && (!Number.isInteger(Number(serviceDraft.startInterval)) || Number(serviceDraft.startInterval) < 1 || Number(serviceDraft.startInterval) > 31536000))}>{serviceCreateBusy ? t("loading") : t("createService")}</button></div></form></Modal>}
...
function LaunchdScheduleOptions({ draft, setDraft, t }) {
  const weekdays = [
  ...
  return <fieldset className="launchd-schedule"><legend>{t("scheduleOptions")}</legend><p className="schedule-combination-help">{t("scheduleCombinationHelp")}</p>
    <div className="schedule-option"><label><input type="checkbox" checked={draft.runAtLoad} onChange={(e) => setDraft((value) => ({ ...value, runAtLoad: e.target.checked }))} /><span><strong>{t("runAtLogin")}</strong><small>{t("runAtLoginHelp")}</small></span></label></div>
    <div className="schedule-option"><label><input type="checkbox" checked={draft.keepAlive} onChange={(e) => setDraft((value) => ({ ...value, keepAlive: e.target.checked }))} /><span><strong>{t("keepRunning")}</strong><small>{t("keepRunningHelp")}</small></span></label></div>
    <div className="schedule-option"><label><input type="checkbox" checked={draft.startInterval !== ""} onChange={(e) => setDraft((value) => ({ ...value, startInterval: e.target.checked ? "300" : "" }))} /><span><strong>{t("startRegularly")}</strong><small>{t("startRegularlyHelp")}</small></span></label>{draft.startInterval !== "" && <div className="schedule-detail"><label><span>{t("intervalSeconds")}</span><input type="number" min="1" max="31536000" step="1" required value={draft.startInterval} onChange={(e) => setDraft((value) => ({ ...value, startInterval: e.target.value }))} /></label></div>}</div>
    <div className="schedule-option"><label><input type="checkbox" checked={draft.calendarIntervals.length > 0} onChange={(e) => setDraft((value) => ({ ...value, calendarIntervals: e.target.checked ? [{ weekday: "", time: "09:00" }] : [] }))} /><span><strong>{t("calendarStart")}</strong><small>{t("calendarStartHelp")}</small></span></label>{draft.calendarIntervals.length > 0 && <div className="calendar-times">{draft.calendarIntervals.map((entry, index) => <div className="calendar-time" key={index}><select aria-label={t("calendarStart")} value={entry.weekday} onChange={(e) => updateCalendar(index, { weekday: e.target.value })}>{weekdays.map(([value, label]) => <option key={value || "daily"} value={value}>{label}</option>)}</select><input aria-label={t("calendarStart")} type="time" required value={entry.time} onChange={(e) => updateCalendar(index, { time: e.target.value })} /><button type="button" aria-label={t("remove")} onClick={() => removeCalendar(index)}>×</button></div>)}<button type="button" className="add-calendar-time" disabled={draft.calendarIntervals.length >= 32} onClick={() => setDraft((value) => ({ ...value, calendarIntervals: [...value.calendarIntervals, { weekday: "", time: "09:00" }] }))}>＋ {t("addTime")}</button></div>}</div>
  </fieldset>;
}
