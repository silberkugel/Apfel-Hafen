...
export function buildCreateArgs(record, imageOverride, nameOverride) {
  ...
  addMany(args, "--env", process.environment);
  ...
  if (config.resources?.cpus) args.push("--cpus", String(config.resources.cpus));
  if (config.resources?.memoryInBytes) args.push("--memory", `${Math.ceil(config.resources.memoryInBytes / 1048576)}M`);
  ...
  for (const port of config.publishedPorts || []) {
