...
function tlsPaths(dataRoot) {
  const directory = join(dataRoot, "tls");
  return {
    directory,
...
export async function ensureFallbackCertificate(dataRoot) {
  const paths = tlsPaths(dataRoot);
...
export async function loadTlsCertificate(dataRoot) {
  const paths = tlsPaths(dataRoot);
  const fallback = await ensureFallbackCertificate(dataRoot);
  try {
...
export async function installCustomCertificate(dataRoot, certificate, privateKey) {
  const paths = tlsPaths(dataRoot);
...
export async function removeCustomCertificate(dataRoot) {
  const paths = tlsPaths(dataRoot);
