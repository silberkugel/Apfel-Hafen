...
export function validateContainerDraft(input, basePath, occupiedPorts = []) {
  ...
  const architecture = optionalText(input?.architecture, 16, "Architektur") || "auto";
  if (!["auto", "arm64", "amd64"].includes(architecture)) throw new Error("Architektur ist ungültig.");
  const dnsDomain = optionalText(input?.dnsDomain, 253, "DNS-Domain");
  if (dnsDomain && !/^(?=.{1,253}$)(?:[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)(?:\.(?:[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?))*$/.test(dnsDomain)) throw new Error("DNS-Domain ist ungültig.");
  const network = optionalText(input?.network, 200, "Netzwerk");
  if (network && (!/^[a-zA-Z0-9][a-zA-Z0-9._-]*(?:,mtu=\d+)?$/.test(network))) throw new Error("Netzwerk ist ungültig.");
  const workdir = optionalText(input?.workdir, 500, "Arbeitsverzeichnis");
  if (workdir && !workdir.startsWith("/")) throw new Error("Arbeitsverzeichnis muss absolut sein.");
  const entrypoint = optionalText(input?.entrypoint, 500, "Entrypoint");
  ...
  const argumentsList = normalizeArguments(input?.arguments);
  ...
  return { name, image, cpus, memoryMb, architecture, rosetta: Boolean(input?.rosetta), dnsDomain, network, interactive: Boolean(input?.interactive), tty: Boolean(input?.tty), workdir, entrypoint, readOnlyRoot: Boolean(input?.readOnlyRoot), useInit: Boolean(input?.useInit), ssh: Boolean(input?.ssh), start: Boolean(input.start), arguments: argumentsList, variables: normalizedVariables, ports: normalizedPorts, volumes: normalizedVolumes };
}
...
export function validateContainerSettings(input, occupiedPorts = []) {
  ...
  return { cpus, memoryMb, arguments: argumentsList, variables: normalizedVariables, ports, volumes };
}

function normalizeArguments(value) {
  const values = Array.isArray(value) ? value : [];
  if (values.length > 128) throw new Error("Es sind höchstens 128 Startargumente erlaubt.");
  return values.map((argument) => {
    if (typeof argument !== "string" || argument.length > 4096 || argument.includes("\0")) {
      throw new Error("Ein Startargument ist ungültig oder zu lang.");
    }
    return argument;
  });
}
...
export function buildNewContainerArgs(draft) {
  ...
  if (draft.architecture && draft.architecture !== "auto") args.push("--arch", draft.architecture);
  if (draft.rosetta) args.push("--rosetta");
  if (draft.dnsDomain) args.push("--dns-domain", draft.dnsDomain);
  if (draft.network) args.push("--network", draft.network);
  if (draft.interactive) args.push("--interactive");
  if (draft.tty) args.push("--tty");
  if (draft.workdir) args.push("--workdir", draft.workdir);
  if (draft.entrypoint) args.push("--entrypoint", draft.entrypoint);
  if (draft.readOnlyRoot) args.push("--read-only");
  if (draft.useInit) args.push("--init");
  if (draft.ssh) args.push("--ssh");
  ...
  args.push(draft.image, ...draft.arguments);
  return args;
}
...
export function parseImageNames(output) {
  ...
  return [...new Set(records.flatMap((record) => [record.configuration?.name, record.name, record.reference, ...(record.names || [])]).filter(Boolean).map(String))]
